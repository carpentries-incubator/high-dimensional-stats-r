...
For more than one predictor variable, we could write 
$\beta_0 + \beta_1 x_{i1} + \dots \beta_p x_{ip}$, but rather than writing out
this long sum, we can also write this using a matrix of predictors $X$
and vector of outcomes $y$ as
...
a mean defined by the product of out predictors and outcomes
...
large ($p=`r round(tab$p.value[[1]], digits = 3)`$).
...
($p=`r round(tab$p.value[[2]], digits = 3)`$).
