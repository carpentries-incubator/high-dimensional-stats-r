...
library("ggplot2")
theme_set(theme_bw())
qplot(x = methyl_mat, binwidth = 0.2, geom= "histogram")
```
...
library("ggplot2")
theme_set(theme_bw())
set.seed(42) # set seed for reproducible "random" points
n <- 10 # number of points per group
sim_feat <- c(rnorm(n, 0), rnorm(n, 3)) # simulate features
sim_phenotype <- (2 * sim_feat) + rnorm(n) # simulate continuous phenotype

# Plot feature against phenotype
qplot(x = sim_phenotype, y = sim_feat,
      xlab = "Age", ylab = "DNA methylation")
```
...
set.seed(42)
n <- 10
# simulate group and features with distinct mean per group
sim_group <- c(rep("A", n), rep("B", n))
sim_feat2 <- c(rnorm(n, 0), rnorm(n, 5))

# Plot feature against group
qplot(x = sim_group, y = sim_feat2, colour = sim_group,
      xlab = "Age", ylab = "DNA methylation",
      geom = "jitter")
```
...
set.seed(66)
n <- 5
sim_group3 <- c(rep("A", n), rep("B", n))
sim_feat3 <- c(rnorm(n, 0), rnorm(n, 1))
qplot(x = sim_group3, y = sim_feat3, colour = sim_group3,
      xlab = "Age", ylab = "DNA methylation",
      geom = "jitter")
```
...
qplot(x=age, y=methyl_mat[1, ], xlab = "Age", ylab = "Methylation level") +
  geom_abline(intercept = coef(lm_age_methyl1)[1], slope = coef(lm_age_methyl1)[2], colour = "blue")
...
# qplot(data = coef_df,
#       x = estimate, y = -log10(p.value),
#       xlab = "Effect size", ylab = "-log10(p)")
```
...
coefs_lm_vs_limma <-
  data.frame(feature = coef_df$feature,
             lm_estimate = coef_df$estimate,
             lm_p.value = coef_df$p.value,
             limma_estimate = toptab_age[coef_df$feature, "logFC"],
             limma_p.value = toptab_age[coef_df$feature, "P.Value"])
             
qplot(data = coefs_lm_vs_limma,
  x = lm_estimate, y = limma_estimate,
...
# Note: this is on raw p.value scale, not log-log
qplot(data = coefs_lm_vs_limma,
      x = lm_p.value, y = limma_p.value,
...
> >    qplot(data = toptab_smoke, x = logFC, y = -log10(P.Value),
> >        xlab = "Effect size", ylab = "-log10(p)")
> >    ```
...
qplot(data = toptab_age_perm, x = logFC, y = -log10(P.Value),
      xlab = "Effect size", ylab = "-log10(p)") +
...
base_bonferroni_plot <- 
    qplot(x = p_raw, y = p_fwer,
          xlab = "Raw p-value", ylab = "Bonferroni p-value")  +
...
> >    qplot(x = p_raw, y = p_fdr,
> >        xlab = "Raw p-value", ylab = "Benjamini-Hochberg p-value")  +
> >        scale_x_log10() + scale_y_log10() +
> >        geom_abline(slope = 1, linetype = "dashed") +
...
> >    qplot(x = p_fdr, y = p_fwer,
> >        xlab = "Benjamini-Hochberg p-value", ylab = "Bonferroni p-value")  +
> >        scale_x_log10() + scale_y_log10() +
> >        geom_abline(slope = 1, linetype = "dashed") +
> >        geom_hline(yintercept = 0.05, linetype = "dashed", color = "red") +
> >        geom_vline(xintercept = 0.05, linetype = "dashed", color = "red")
