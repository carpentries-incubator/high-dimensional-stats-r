...
Often researchers want to reduce dimensionality in high-dimensional datasets to
look for relationships among variables, identify clusters among subgroups of
data points and summarise the dataset using a smaller number of representative
variables.
...
The principal components are single variables which are calculated using a
linear combination of several (continuous) variables from the original dataset
...
contribution original variables make to the calculation of principal components
is represented in *principal component loadings*.  
...
large the number of plots needed to examine the data this way quickly become
...
* It is sensitive to the scale at which input variables are measured.
  (hence the need for standardisation in most cases). If input variables a
  measured at different scales, loadings will be largest for variables with
  greatest variance which is related to scales of measurement.
...
* It can be difficult to interpret the meaning of the principal components,
  especially when including them in further analysis (e.g. inclusion in a linear
  regression).
...
> analyses in which each observation has both at least one independent variables
...
A PCA is carried out by calculating a matrix of Pearson's correlations from the
original dataset which shows how each of the variables in the dataset relate to
each other.
...
We will create a subset of the data including only clinical variables we want to
use in the PCA.
...
variation in the data, without having extra components that are unnecesary?
...
weight of loadings can be observed via the red arrows associated with each of
...
(`pca.pros$sdev`) times square root the number of observations.


# Using PCA to analyse gene expression data 

In this section you will carry out your own PCA using the `PCAtools` package
and gene expression data to try out the topics covered above. 
...
We will now load the microarray breast cancer gene expression data and
...
> function (type `?pca` in R). Remove the lower 20% of principal components
...
> > pc$loadings[27, ]
> > ```
> > The `pca` uses as inputs a matrix containing continuous numerical data
...
> > for each variable (probe), principal components and amount of variance
> > explained by each principal component.Rotated data shows principal
> > component scores for each sample and each principal component. Loadings
> > show principal component loading (or weight) each probe makes to each
> > principal component. 
> {: .solution}
{: .challenge}
...
> > including genetic information as clear relationships between groups is
