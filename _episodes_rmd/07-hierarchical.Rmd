...
example, a cut at height 10 produces `r length(unique(cutree(clust, h=10)))` clusters for the dendogram in Challenge 1,
...
> > ```{r h-k-ex-plot}
> > plot(clust)
> > ## create horizontal line at height = 36
> > abline(h = 36, lty = 2)
> >
> > cutree(clust, h = 36)
> > cutree(clust, k = 9)
> >
> > nine_cut <- cutree(clust, h = 36)
> >
> > library(dplyr)
> > example_cl <- mutate(data.frame(methyl_mat), cluster = nine_cut)
> > count(example_cl, cluster)
> >
> > library(ggplot2)
> > ggplot(example_cl, aes(x=cg01644850, y = cg01656216, color = factor(cluster))) + geom_point()
> > ```
> > 
> > Nine clusters (`k = 9`) gives similar results to `h = 36`. You can plot a
> > horizontal line on the dendrogram at `h = 36` to help identify
