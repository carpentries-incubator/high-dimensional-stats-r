...
> 
> b) Describe what variables were measured (hint: see the `names()` and `head()` functions)
> 
> c) Plot the relationship between the variables (hint: see the `pairs()` function). 
> Why does it become more difficult to plot relationships between pairs of variables with
...
> > names(Prostate)  #print column names
...
observations per feature is low. The result of fitting a best fit line through
