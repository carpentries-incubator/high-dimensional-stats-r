...
Throughout this episode, we will carry out a PCA using the Bioconductor package **`PCAtools`**.
This package provides several functions that can be used to explore data via PCA and
...
options for performing PCA, including a base R function, `prcomp()`. Equivalent functions for 
the common implementations are summarised below.
...
Let's use the **`PCAtools`** package to perform PCA on the scaled `prostate` data. The input
data (`pros2`) is in the form of a matrix and we set the `scale = TRUE` argument
to standardise the variables to have a mean 0 and standard deviation of 1. 
...
In this section, we will work through Challenges and you will apply your own PCA to the high-dimensional gene expression data using what we have learnt so far. Let's load the **`PCAtools`** package and the data.
