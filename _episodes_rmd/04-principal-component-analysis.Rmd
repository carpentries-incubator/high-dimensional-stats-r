...
Note that, although we focus on PCA implementation using **`PCAtools`**, there are several other
...
Let's use the **`PCAtools`** package to perform PCA on the scaled `prostate` data. First, let's load the **`PCAtools`** package and the data.
...
matrix for use within the **`PCAtools`** package. We can do this by setting the 
`transposed = TRUE` argument. We also set the `scale = TRUE` argument
to standardise the variables to have a mean 0 and standard deviation of 1 as above.

```{r prcomp}
pca.pros <- pca(t(pros2), scale = TRUE, center = TRUE, transposed = TRUE)
...
(p = 5) principal components. In this example, the first principal component, PC1, explains approximately
...
where initially, the variance drops
...
The scree plot (blue bars) show that the first principal component explains most of the
variance in the data (>50%) and each subsequent principal component explains
less and less of the total variance. The red line displays the cumulative variance explained
by the principal components and shows that the first two principal components
...
PCAtools::biplot(pca.pros, xlim = c(-0.3, 0.3))
```
...
> > ```
> > The red line shows the cumulative percentage of explained
> > variation with increasing principal components. The first 
> > principal component explains around 33% of the 
...
> > components explain very little variation. Looking at the scree plot, the
