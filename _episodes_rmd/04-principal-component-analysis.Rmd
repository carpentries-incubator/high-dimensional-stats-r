...
is also called a scree plot. NB, the amount of variance accounted for by a principal
...
rapidly with each additional PC. But then there is an “elbow” after which the
variance decreases more slowly. This is sometimes interpreted as structural variance
that is relevant and should be retained versus noise which may be discarded. 
...
*principal component scores* and the *loadings*. The matrix of PC scores
...
The rotation matrix has as many rows as there are features in the original data.
It contains information about how the (usually centered and scaled) original
data relate to the PC scores.
...
The PC scores are obtained by carrying out matrix multiplication of the
(usually centered and scaled) original data time the loadings. The following
...
> abline(h=0, v=0, lty=2)
> par(mfrow=c(1,1)
> # NB, axis orientations may be swapped but the relative positions of the dots should be the same in both plots.
> ```
{: .callout}
