...
```{r var-hist, fig.cap="Histograms of two variables from the Prostate data set.", fig.cap="Side-by-side histograms of two variables from the dataset, lweight on the left and lbph on the right. The histogram for the lweight data ranges from around 2 to 6 on the x axis, while the histogram for the lbph ranges from around -2 to 3 on the x axis."}
apply(pros2, 2, var)
par(mfrow = c(1, 2))
...
```{r, echo = FALSE}
# Get proportions of variance explained by each principal component (rounded to 2 DP)
...
contribute to explain the remaining variance. In this case, the 'elbow' of the scree plot appears
...
```{r stats-biplot, fig.cap="Scatter plot of the first two principal components with observations numerically labelled. The loadings are down by red arrows.", fig.cap="Scatter plot of the second principal component versus the first principal components. Observations as points, numerically labelled. The loadings are shown by red arrows, each labelled by their associated variable."}
...
> > #Many principal components explain a very small amount of the total variance in the data
> > #Remove the lower 20% of principal components with lower variance
...
> output of the scree plot in terms of proportion of variance in data explained
...
```{r pairsplot, fig.cap="Multiple biplots produced by pairsplot().", fig.cap="An upper triangular grid of scatter plots of each principal component versus the others."}
pairsplot(pc)
```
