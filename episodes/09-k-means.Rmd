...
We will create some example data that form three clusters ('a', 'b', 'c'). We plot these clusters 
and place ellipses around each group using the `stat_ellipse` function in `ggplot`.
...
and partition these data into three groups according to data values. We then plot these 
data and their allocated clusters and put ellipses around the clusters.
...
> of clustering. Some strategies to deal with this problem are:
 ...
 > - Using the K-means++ algorithm to choose initial values.
