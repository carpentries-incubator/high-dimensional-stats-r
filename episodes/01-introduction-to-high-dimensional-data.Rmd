...
*High-dimensional data* are defined as data with a large number of features (variables observed),
$p$. In recent years, advances in information technology have allowed large amounts of data to
be collected and stored with relative ease. As such, high-dimensional
data have become more common in mant scientific fields, including the biological sciences, 
where datasets in subjects like genomics and medical sciences are often tall (with large $n$) and wide
(with large $p$). For example, data collected from hospital patients may record symptoms,
blood test results, behaviours, and general health, resulting in datasets with
large numbers of features. An example of what high-dimensional data might look like 
...
are no longer appropriate when we have many features and 
relatively few observations. In this lesson, we will learn alternative methods 
