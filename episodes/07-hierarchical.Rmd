...
highlight brances in dentrograms. In this example, we calculate a distance matrix between
...
```{r crescents}
# These two functions to help us make crescents. Don't worry it you do not understand all the code.
# The importent bit is the object "cres", which consists of two columns (x and y coordinates).
...
run hierarchical clusterign with the default arguments, we get this:
...
> > plot(cres, cressClassSingle, main="single")
> > ```
> > ```{r plot-clust-wardD}
