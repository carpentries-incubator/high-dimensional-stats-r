...
```{r hclustfig1, echo=FALSE, out.width="500px", fig.cap="Example data showing two clusters of observation pairs"}
knitr::include_graphics("../fig/hierarchical_clustering_1.png")
```

```{r hclustfig2, echo=FALSE, out.width="500px", fig.cap="Example data showing fusing of one observation into larger cluster"}
...
```{r plot-clust-method, fig.cap="A dendrogram of the methylation data, with boxes showing different possible cut points.", fig.alt="A line plot depicting a dendrogram --- a tree structure representing the hierarchical structure of the data. The data broadly fit into three clusters, with one sample (14) being quite dissimilar to all others, and the rest of the data comprising two other clusters (one larger than the other). A set of coloured boxes depict cut points to divide the data into two or three clusters."}
...
```{r plot-coloured-branches, fig.cap="A dendrogram of the methylation data, with branches coloured by cluster.", fig.alt="A line plot depicting a dendrogram --- a tree structure representing the hierarchical structure of the data. The data broadly fit into three clusters, with one sample (14) being quite dissimilar to all others, and the rest of the data comprising two other clusters (one larger than the other). A set of coloured boxes depict cut points to divide the data into two or three clusters."}
library("dendextend")
...
```{r crescents, fig.cap="A scatter plot of synthetic data.", fig.alt="A scatter plot of synthetic data, comprising two variables, with points forming two crescent-shaped clusters."}
...
```{r cresClustDefault, fig.cap="A scatter plot of synthetic data coloured by cluster.", fig.alt="A scatter plot of synthetic data, comprising two variables, with points forming two crescent-shaped clusters. Points are coloured based on hierarchical clustering, with two clusters, though these do not correspond to the two crescent-shaped clouds."}
...
```{r heatmap-cor-example, fig.cap="Heatmap of synthetic data.", fig.alt = "A heatmap of synthetic data, with red indicating high values and blue indicating low values. Samples A and B cluster together, while sample C clusters separately."}
Heatmap(as.matrix(cor_example))
```

We can see that more clearly if we do a line plot:
```{r lineplot-cor-example, fig.cap="Line plot of synthetic data.", fig.alt = "A line plot of synthetic data. Red and green lines follow the same pattern but are separated by vertical distance. A blue line is close in vertical space to the red line, but follows a dissimilar pattern."}
...
```{r clust-cor-cor-example, fig.cap="Demo", fig.alt="No"}
...
```{r hclust-fig4, echo=TRUE, fig.cap="Dunn index continued"}
