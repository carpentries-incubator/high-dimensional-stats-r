...
our features as `sample_a`, just offset by 5. Then we convert the dataframe into a matrix:

```{r sample-c}
cor_example$sample_c <- cor_example$sample_a + 5
cor_example <- as.matrix(cor_example)
```
...
Heatmap(cor_example, clustering_distance_cols = "correlation")
...
greater increases the value of the Dunn index in this example. In this example,
decreasing `h` below 20 can give an infinite Dunn index.
