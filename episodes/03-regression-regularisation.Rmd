...
> > error of the predicted values and true test data.
> > ```{r testerror}
...
when $\lambda$ is large, we want to penalise large coefficients, i.e., we care a lot about the size of the coefficients
...
regression gives us a better prediction compared to the model with no regularisation. 
...
# Extract the coefficients corresponding to the lowest mean squared error from cross-validation
coefl <- coef(lasso, lasso$lambda.min)
selected_coefs <- as.matrix(coefl)[which(coefl != 0), 1] ; selected_coefs
```

We can see that cross-validation has selected a value of $\lambda$ resulting in `r length(selected_coefs)-1` features and the intercept. We can cross-reference these features with the Hovarth data to compare the features to the Hovarth signature:
...
```{r heatmap-lasso, echo = FALSE, fig.cap="Heatmap showing methylation values for the selected CpG and how the vary with age.", fig.alt="Overall, we observe either increasing or decreasing methylation patterns as a function of age."}
